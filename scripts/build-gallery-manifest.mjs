...
//
// Pure metadata work — no ImageKit calls. Run before scripts/upload-gallery.mjs.
//
...
function parseFilename(filename) {
	...
	const type = TYPE_TOKEN_MAP[rest[0]];
	rest = rest.slice(1);
...
// All images become .jpg in dest (sharp resize+recode handles q85 + max 2000px).
// All videos become .mp4 in dest (ffmpeg transcodes to H.264 + AAC, max 1920px).
function buildDestFilename(parsed, originalName) {
	...
	// Replace brand prefix in the rebuilt name with canonical slug (e.g. dhwani_bansal_jewellery → dbj)
	const sorted = [...BRAND_PREFIX_ALIASES].sort((a, b) => b[0].length - a[0].length);
	for (const [prefix, slug] of sorted) {
...
async function main() {
	...
	// would silently overwrite each other on R2 (PutObjectCommand has no "no-overwrite"
	// flag). Surface duplicates so operator can rename source files.
	const seenDests = new Map(); // destPath → originalPath
	const collisions = [];
	for (const m of manifest) {
