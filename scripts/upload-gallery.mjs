...
//
// Reads scripts/gallery-manifest.json and uploads every `include: true` entry to
// ImageKit at its destPath. Idempotent: tracks success in scripts/upload-progress.json
// and skips already-uploaded files on re-run.
//
// Pipeline:
// - Images (.jpg/.jpeg/.png/.webp/.avif/.gif): sharp resize to fit max 2000x2000 +
//   re-encode as JPG q85 (mozjpeg). Keeps storage tight; site never renders above ~1200px.
// - Videos (.mp4/.mov/.webm): ffmpeg transcode to H.264 + AAC, scale to fit max
//   1920x1920, CRF 23, max 4 Mbps. Free-tier per-file 100 MB cap is enforced upstream
//   by file_size_limit defaults.
//
// On completion, emits scripts/gallery-items.generated.ts — a typed array of
// GalleryItem entries ready to fold into src/data/gallery.ts.
//
...
async function loadEnv() {
	...
	for (const line of content.split(/\r?\n/)) {
		...
		// Strip surrounding quotes — common .env-file convention but breaks our consumers
		// when present (e.g. R2_PUBLIC_URL="https://..." would 404 with literal quotes).
		const val = trimmed
			.slice(idx + 1)
			.trim()
			.replace(/^"(.*)"$/, '$1')
...
async function processVideo(srcBuf, sourceExt) {
	...
	try {
		...
		// Generate a poster image at 1s offset (or 0s if shorter), max 800px wide.
		// Used as the <video> tag's poster — replaces ImageKit's ik-thumbnail.jpg trick.
...
async function main() {
	...
	const endpoint = process.env.IMAGEKIT_URL_ENDPOINT || 'https://ik.imagekit.io/nexailabs';
	...
	// rather than try-and-fail per video. Image-only manifests can run without R2.
	...
	// real width/height (older runs before ffprobe was added). Only works on the
	// machine that has the source files at originalPath.
	let backfilled = 0;
