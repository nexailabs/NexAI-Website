...
Single branch: `main`. Branch protection requires 1 review and CI green.

Conventional commits — type(scope): subject. Common types: `feat`, `fix`, `chore`, `refactor`, `docs`, `test`. Scopes: `home`, `prompts`, `apps`, `blog`, `brand`, `seo`, `repo`.
...
Pre-commit hooks (Husky + lint-staged) run `eslint --fix` and `prettier --write` on staged files. CI runs `lint`, `type-check`, `build`. Don't bypass hooks (`--no-verify`).
...
- `audits/` — periodic audit reports (architecture, security, SEO, branding, copy, repo cleanup). Read-only research; not source of truth.
